...
def store_top_n_keywords():
    title_gossip_days_1 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_article_title_gossip_1").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    title_gossip_days_3 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_article_title_gossip_3").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    title_gossip_days_7 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_article_title_gossip_7").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    title_politics_days_1 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_article_title_politics_1").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    title_politics_days_3 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_article_title_politics_3").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    title_politics_days_7 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_article_title_politics_7").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    ...
    comment_gossip_days_1 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_comments_gossip_1").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    comment_gossip_days_3 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_comments_gossip_3").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    comment_gossip_days_7 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_comments_gossip_7").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    comment_politics_days_1 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_comments_politics_1").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    comment_politics_days_3 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_comments_politics_3").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
    comment_politics_days_7 = extract_top_n_keywords(
        text_data=json.loads(
            redis_conn.get(name="past_n_days_comments_politics_7").decode("utf-8")
        ),
        n_keywords=NUM_KEYWORDS,
    )
...
def retrieve_top_n_keywords(target_collection: str, n_days: int, source: str):
    ...
    return json.loads(
        redis_conn.get(f"keyword_{source}_{target_collection}_{n_days}").decode("utf-8")
    )
